...
class BaseInstanceList(ABC):
    ...
    def _render(self, table : str, actions : Actions, window) -> None:
        ...
        end_index = min(len(self.state.instances) - 1, self.state.current + (lines_before_after // 2) + difference)
        displayed = body[start_index:end_index+1]
        for i, line in enumerate(displayed):
            true_i = i + start_index
            y_pos = i + 2
            attr = curses.A_STANDOUT if self.state.instances[true_i] in self.state.selected else curses.A_NORMAL
        ...
        processed_actions = sorted(processed_actions.items(), key=lambda x : x[1][1], reverse=True)
        action_strings = []
...
class ConsumableList(BaseInstanceList):
    ...
    def tabulate(self) -> str:
        ...
        table_instances = [[row+1, i.id, i.type, i.name, i.parts, i.rating, i.completions, i.status.name,
                      datetime.fromtimestamp(i.start_date).strftime(self.date_format) if i.start_date else i.start_date,
